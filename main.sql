...
SELECT students.fname, students.lname, courses.cname
FROM (students INNER JOIN registration ON students.sid = registration.sid) INNER JOIN courses ON courses.cid = registration.cid
WHERE courses.cname = 'organic chemistry';

.print \n\n-------------------------------------Problem 4---------------------------------------
SELECT students.fname, students.lname, courses.cname
FROM (students INNER JOIN registration ON students.sid = registration.sid) INNER JOIN courses ON courses.cid = registration.cid
WHERE students.fname = 'Tad';

.print \n\n-------------------------------------Problem 5---------------------------------------
SELECT teachers.fname,teachers.lname,students.fname,students.lname
FROM ((students INNER JOIN registration ON students.sid = registration.sid) INNER JOIN courses ON courses.cid = registration.cid) INNER JOIN teachers ON teachers.tid = courses.tid
WHERE teachers.fname = 'beth';